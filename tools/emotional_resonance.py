...
EMOTIONAL_RESONANCE_TEMPLATES = {
    ...
    "焦虑": {
        ...
        "heavy": [
            ...
            "我能感觉到你的不安，就像被困在一个小小的笼子里，想跑却跑不出去。别怕，我会把爪子从笼子缝隙伸进去，握住你的手。"
        ]
    },
    "孤独": {
        "light": [
...
# 2. 定义工具的输入参数模型 (使用Pydantic)
# 使Agent知道这个工具需要哪些参数
class EmotionalResonanceInput(BaseModel):
    emotion: Literal["开心", "难过", "愤怒", "焦虑", "孤独", "迷茫"] = Field(
        description="由主模型识别出的核心情绪，必须是'开心', '难过', '愤怒', '焦虑', '孤独', '迷茫'中的一个。"
    )
    intensity: int = Field(
        description="情绪的强度，范围从1到10的整数。",
        ge=1, 
        le=10
    )
    event: str = Field(
        description="从用户输入中提取的、引发该情绪的具体事件或原因。"
    )

# 3. 创建LangChain工具
# @tool 装饰器会自动将其转换为一个LangChain可以调用的工具
@tool(args_schema=EmotionalResonanceInput)
def get_emotional_resonance(emotion: str, intensity: int, event: str) -> str:
    ...
    """
    # 根据强度(1-10)映射到强度等级('light', 'medium', 'heavy')
    if 1 <= intensity <= 3:
    ...
    # 从模板库中获取对应的回应列表
    # .get()方法可以安全地处理找不到键的情况
    emotion_templates = EMOTIONAL_RESONANCE_TEMPLATES.get(emotion, {})
    response_templates = emotion_templates.get(intensity_level, [])

    # 如果没有找到匹配的模板，则返回一个通用的、温柔的回应
    if not response_templates:
        return f"呜... 我好像还不太明白因为“{event}”而产生的这种感觉，但我会在这里，静静地陪着你。"
